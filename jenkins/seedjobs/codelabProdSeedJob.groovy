    ...
    publishers {
      downstreamParameterized {
        trigger("${jobName}-2-docker-start") {
          parameters {
            predefinedProp("WORK_AROUND", "WORK_AROUND")
          }
        }
      }
    }
  ...
  job("${jobName}-2-docker-start") {
    ...
    steps {
      steps {
        ...
        shell("sudo /usr/bin/docker run --network=host --restart always -v /share/data/${dockerImageNameOfDb}:/data/db --env-file ./env -d --name ${dockerImageNameOfDb} -p=${dockerPortMappingOfDb} ${registryUrl}/codelab/${dockerImageNameOfDb}")
