  ...
  job("${jobName}-2-docker-start") {
    ...
    steps {
      steps {
        ...
        shell("sudo /usr/bin/docker run --restart always -v ~/data/${dockerImageNameOfDb}:/data/db --env-file ./env -d --name ${dockerImageNameOfDb} -p=${dockerPortMappingOfDb} ${registryUrl}/codelab/${dockerImageNameOfDb}")
